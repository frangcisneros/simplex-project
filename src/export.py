...
def export_to_pdf(result: dict, filename: str):
    ...
    left_margin = 30
    right_margin = 30
    available_width = 540 -left_margin - right_margin  # ancho utilizable para todo el documento(tener en cuenta márgenes)
    ...
    if "problem" in result:
        ...
        if objective_str.startswith("Maximizar"):
            ...
            expr = add_space(expr)
            problem_content.append(Paragraph(f"<b>Maximizar:</b> <b><font color='{highlight_color}'>{expr}</font></b>", styles["Normal"]))
        elif objective_str.startswith("Minimizar"):
            expr = objective_str[len('Minimizar '):]
            expr = add_space(expr)
            problem_content.append(Paragraph(f"<b>Minimizar:</b> <b><font color='{highlight_color}'>{expr}</font></b>", styles["Normal"]))
        else:
            expr = add_space(objective_str)
            problem_content.append(Paragraph(f"<font color='{highlight_color}'>{objective_str}</font>", styles["Normal"]))
        ...
        for constr in problem["constraints_str"]:
            ...
            problem_content.append(Paragraph(f"<b><font color='{highlight_color}'>{constr_fmt}</font></b>", styles['Normal']))
        
        # --- Crear la tabla de una celda ---
        problem_table = Table([[problem_content]], colWidths=[available_width * 0.95])  # centrado dentro del ancho útil
    ...
    if status == "optimal":
        ...
        solution_content.append(Paragraph(f"<b>Valor óptimo:</b> <b><font color='{highlight_color}'>z={optimal_value:.2f}</font></b>", styles['Normal']))
        solution_content.append(Spacer(1, 4))  # Espaciado extra antes de las variables

        solution = result.get("solution", {})
        if solution:
            vars_str = ", ".join(f"<b><font color='{highlight_color}'>{var}={val:.2f}</font></b>" for var, val in solution.items())
        ...
        solution_content.append(Paragraph(f"Error: {result.get('message','')}", styles['Heading3']))

    # Crear tabla de 1 celda si hay contenido
    if solution_content:
        solution_table = Table([[solution_content]], colWidths=[available_width * 0.95])  # # mismo ancho que el bloque del problema
    ...
    if "steps" in result:
        ...
        for idx, step in enumerate(steps):
            ...
            styles_list = [
                ...
                ('LINEABOVE', (0, m), (-1, m), 1, colors.black)     # línea superior de la última fila
            ]
            ...
            if pivot_info:
                ...
                styles_list += [
                    ('BACKGROUND', (entering_col, 1), (entering_col, m), colors.lavender),   # Columna pivote
                    ('BACKGROUND', (1, leaving_row), (-1, leaving_row), colors.lavender),    # Fila pivote
                    ('BACKGROUND', (entering_col, leaving_row), (entering_col, leaving_row), colors.lightblue),  # Celda pivote
                    ('TEXTCOLOR', (entering_col, leaving_row), (entering_col, leaving_row), colors.darkblue),
                    ('BACKGROUND', (0, 1), (0, m-1), colors.whitesmoke),    # VB
                    ('BACKGROUND', (0, m), (0, m), colors.lightgrey)        # Última fila VB
                ]
            ...
            if idx == len(steps) - 1:
                ...
                for row_idx, vb in enumerate(basic_vars):
                    if isinstance(vb, int) and vb < n_original_vars:  # Filtra VB reales
                        styles_list.append(('BACKGROUND', (0, row_idx+1), (0, row_idx+1), colors.lavender))  # VB
                        styles_list.append(('BACKGROUND', (-1, row_idx+1), (-1, row_idx+1), colors.lavender))  # RHS
            ...
            arrow_para = Paragraph(arrow_text, ParagraphStyle(
                name='ArrowStyle',
                fontSize=14,
                alignment=TA_CENTER,
            ...
            arrow_para = Paragraph(
                ...
                ParagraphStyle(name='ArrowStyle', alignment=TA_CENTER, textColor="#7F7F7F")
            )
            ...
            wrapper_table.setStyle(TableStyle([
                ('VALIGN', (0,0), (0,-1), 'MIDDLE'),  # Columna 0: flecha centrada verticalmente
                ('VALIGN', (1,0), (1,-1), 'TOP'),      # Columna 1: tabla mantiene alineación superior
