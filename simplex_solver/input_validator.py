...
class InputValidator:
    """
    Clase para validar los parámetros de entrada en problemas de programación lineal.
    Proporciona métodos para verificar la validez de la función objetivo, las restricciones
    y la consistencia general del problema.
    """
    ...
    def validate_problem(
        ...
        Returns:
            Tuple (es_válido, mensaje_error): Indica si el problema es válido y un mensaje de error en caso contrario.
        """
    ...
    def _validate_objective_function(c: List[float]) -> Tuple[bool, str]:
        ...
        Returns:
            Tuple (es_válido, mensaje_error): Indica si la función objetivo es válida y un mensaje de error en caso contrario.
        """
        if not c:
    ...
    def _validate_constraints(
        A: List[List[float]], b: List[float], constraint_types: List[str], num_vars: int
    ) -> Tuple[bool, str]:
        """
        ...
        Args:
            A: Matriz de coeficientes de restricciones.
            b: Lista de términos independientes de restricciones.
            constraint_types: Lista de tipos de restricciones ('<=', '>=', '=').
            num_vars: Número de variables en el problema.

        Returns:
            Tuple (es_válido, mensaje_error): Indica si las restricciones son válidas y un mensaje de error en caso contrario.
        """
        if not A:
        ...
        for i, (row, rhs, const_type) in enumerate(zip(A, b, constraint_types)):
            ...
            if const_type not in ["<=", ">=", "="]:
                return (
                    False,
                    f"Restricción {i+1}: tipo '{const_type}' no válido. Use '<=', '>=' o '='",
                )
            ...
            if len(row) != num_vars:
                return (
                    False,
                    f"Restricción {i+1}: número de coeficientes ({len(row)}) no coincide con variables ({num_vars})",
                )
            ...
            if const_type == "=" and rhs < 0:
                return (
                    False,
                    f"Restricción {i+1} de igualdad no puede tener término independiente negativo",
                )

        return True, "Restricciones válidas"

    @staticmethod
    def _validate_problem_consistency(
        A: List[List[float]], b: List[float], constraint_types: List[str]
    ) -> Tuple[bool, str]:
        """
        ...
        Returns:
            Tuple (es_válido, mensaje_error): Indica si el problema es consistente y un mensaje de error en caso contrario.
        """
        ...
        for i in range(len(A)):
            for j in range(i + 1, len(A)):
                ...
                if (
                    A[i] == A[j]
                    and constraint_types[i] == "<="
                    and constraint_types[j] == ">="
                    and b[i] < b[j]
                ):
        ...
        if all_positive_constraints and all_negative_coeffs and positive_rhs:
            return (
                False,
                "El problema parece infactible: todas las restricciones son >= con coeficientes no positivos y RHS positivos",
            )
    ...
    def _is_finite_number(x: float) -> bool:
        ...
        Args:
            x: Número a verificar.

        Returns:
            bool: True si el número es finito, False en caso contrario.
        """
        return (
            isinstance(x, (int, float)) and abs(x) != float("inf") and x == x
        )  # x == x verifica que no sea NaN
    ...
    def validate_solution_feasibility(
        ...
        Returns:
            Tuple (es_factible, lista_de_errores): Indica si la solución es factible y una lista de errores en caso contrario.
        """
        errors = []
