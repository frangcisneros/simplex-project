...
class TestPDFExport(unittest.TestCase):
    ...
    def test_pdf_generation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertGreater(os.path.getsize(pdf_path), 1000, "El PDF generado está vacío o incompleto.")
            ...
            self.assertGreater(len(self.result["steps"]), 0, "No se encontraron pasos en el historial.")

            # Comprobar que el último tableau esté presente
            last_step = self.result["steps"][-1]
            self.assertIn("tableau", last_step, "No se encontró el tableau final en el último paso.")

    @unittest.skip("Se desactiva temporalmente hasta corregir la resolución de minimización.")
